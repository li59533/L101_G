...
extern uint8_t drawFloat(uint8_t row,uint8_t col,float f,uint8_t format,uint8_t bytes,uint8_t color);
extern uint8_t drawInt32(uint8_t row,uint8_t col,uint32_t f,uint8_t deci,uint8_t color);
...
#define lcd_cs_clr() {GPIO_ResetBits(MKL_PORTD,4);}
#define lcd_cs_set() {GPIO_SetBits(MKL_PORTD,4);}
//#define lcd_reset_clr() {GPIO_ResetBits(MKL_PORTE,30);}
//#define lcd_reset_set() {GPIO_SetBits(MKL_PORTE,30);}
//#define lcd_reset_clr() {GPIO_ResetBits(MKL_PORTA,3);}
//#define lcd_reset_set() {GPIO_SetBits(MKL_PORTA,3);}
#define lcd_rs_clr() {GPIO_ResetBits(MKL_PORTD,7);}
#define lcd_rs_set() {GPIO_SetBits(MKL_PORTD,7);}
