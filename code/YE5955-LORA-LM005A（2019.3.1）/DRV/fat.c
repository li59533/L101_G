...
INT8U SD_WriteMultiSectors( INT16U sectors, INT16U num ,INT8U *buff );
INT8U SD_ReadMultiSectors( INT16U sectors, INT16U num, INT8U *buff );

//函数指针指向sd卡的读写函数
uint8_t (* FAT_ReadSector)(uint32_t sector, uint32_t dwSector, uint8_t * buffer)=SD_ReadMultiSectors;
uint8_t (* FAT_WriteSector)(uint32_t sector, uint32_t dwSector, uint8_t * buffer)=SD_WriteMultiSectors;
...
//	printf("0x%x",buffer[i]); printf(" ");
	...
	{
	printf("ERROR -- SD card no bootsect!\n");
		return 1;
	}
...
#if FAT_DEBUG
	printf("FirstDirSector:%x",FirstDirSector);	  printf("\n");
	printf("RootDirSectors:%x:",RootDirSectors);	  printf("\n");
	printf("RootDirCount:%x",RootDirCount);		  printf("\n");
	printf("FirstFATSector:%x",FirstFATSector);	  printf("\n");
	printf("SectorsPerClust:%x",SectorsPerClust); printf("\n");
	printf("BytesPerSector:%x",BytesPerSector);	  printf("\n");
	printf("FirstDataSector:%x",FirstDataSector); printf("\n");

	printf("bpbSectors:%x",bpb->bpbSectors); printf("\n");
	printf("filesystem version:%x",bpb->bpbFSVers); printf("\n");
#endif
	printf("FAT Init O.K\n");
	return 0;
}
...
uint16_t FAT_FindFreeItem(uint32_t cluster, struct FileInfoStruct *FileInfo)
{
	...
	else//other folders//当这是一个其它目录时，程序暂且没考虑到目录内没有空余项是需要另找一个簇来记录
	{
		...
		while(1)
		{
			sector=(uint16_t)FirstDataSector+(tempclust-2)*(uint16_t)SectorsPerClust;//calculate the actual sector number
			//buffer=malloc(512);//apply memory
			if(buffer==0)return 1;//if failed
...
uint8_t FAT_DisDir(uint8_t *dir)
{
		...
		for(cnt=0;cnt<RootDirSectors;cnt++)
		{
			...
			for(offset=0;offset<512;offset+=32)
			{
				...
				if((item->deName[0] != '.') && (item->deName[0] != 0x00) && (item->deName[0] != 0xe5) && (item->deAttributes != 0x0f))
				{
					...
					if((item->deAttributes & 0x10)==0) printf(".");
	...
	else//other folders
	{
		...
		while(1)
		{
			sector=(uint16_t)FirstDataSector+(uint16_t)(tempclust-2)*(uint16_t)SectorsPerClust;//calculate the actual sector number
			//buffer=malloc(512);//apply memory
			if(buffer==0)return 1;//if failed
			for(cnt=0;cnt<SectorsPerClust;cnt++)
			{
				...
				for(offset=0;offset<512;offset+=32)
				{
					...
					if((item->deName[0] != '.') && (item->deName[0] != 0x00) && (item->deName[0] != 0xe5) && (item->deAttributes != 0x0f))
					{
						...
						{
							//printf("%c",item->deExtension[i]);
							uart0SendChar(item->deExtension[i]);
						}
					}
				}
			}
			//free(buffer);//release
			tempclust=FAT_NextCluster(tempclust);//next cluster
			if( tempclust == 0xffffff8 )return 1;
...
unsigned char FAT_SelectOneItem(uint32_t cluster,struct FileInfoStruct *FileInfo)
{
	...
	if(cluster==0 && FAT32_Enable==0)// root directory
	{
		...
		for(cnt=0;cnt<RootDirSectors;cnt++)
		{
			if((FirstDirSector+cnt,1,buffer))
			{
				//free(buffer);
				return 1;
			}
...
uint8_t FAT_MkDir(uint8_t * dir)
{
	...
	sector=(uint16_t)FirstDataSector+(uint16_t)(cluster-2)*(uint16_t)SectorsPerClust;
...
unsigned char FAT_Read(uint32_t pointer, uint32_t size)
{
	...
	sector=(uint16_t)FirstDataSector+(uint16_t)(tempclust-2)*(uint16_t)SectorsPerClust;
	...
	while(size)
	{
		...
		else
		{
			...
			if(j==SectorsPerClust)
			{
				...
				if((FAT32_Enable == 0 && tempclust == 0xffff) || tempclust == 0x0ffffff8 || tempclust == 0x0fffffff)return 1;
				sector=(uint16_t)FirstDataSector+(uint16_t)(tempclust-2)*(uint16_t)SectorsPerClust;//reculculate the sector
			}
			else 
				;//free(buffer);
			size-=512;
...
{
	uint8_t name[11];
	uint8_t *p=(uint8_t*)dir; //lh 2013-12-16
	uint8_t 	deep=0;
	uint8_t 	i,j;
	uint32_t	cluster=0;
	...
	if(FAT_FindItem(cluster,name, File_Info))return 1;//find the file --czx
	cluster = File_Info->StartCluster;
	return	cluster;
}
...

{
	...
	if(cluster == 1)
	{
		////打开目录失败.
#if(FAT_DEBUG==1)
		printf("FAT_GetFile:");uart0SendChar(dir)；printf("Err!\n");
#endif
		return 1;
	}
	////
...
//	uint32_t	Cnt = 0;
	...
	//printf("Preparing Load File");printf("to sdram:");_DBH32((uint32_t)Buf);_DBG("\n");

	cluster=FAT_OpenFile(dir,&FileInfo);

	if(cluster==0 || FileInfo.Size == 0)
	{
		printf("FAT Load File"); printf(" Err!\n");
		return 0;
	}
	size=0;
