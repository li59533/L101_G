...
void RTC_ITDMAConfig(RTC_ITDMAConfig_Type config, bool status)
{
    ...
    switch(config)
    {
        ...
        case kRTC_IT_TimeSeconds:
            ...
            (status)?(RTC_IER |= RTC_IER_TSIE_MASK):(RTC_IER &= ~RTC_IER_TSIE_MASK);
            break;
...
void RTC_IRQHandler(void)
{
    ...
    if((RTC_IER & RTC_IER_TOIE_MASK) && (RTC_SR & RTC_SR_TOF_MASK))
    {
        RTC_TSR = RTC_TSR;
    }
    if((RTC_IER & RTC_IER_TAIE_MASK) && (RTC_SR & RTC_SR_TAF_MASK))
    {
        RTC_TAR = RTC_TAR;
    }
}
